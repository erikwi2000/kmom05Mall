...
UPDATE user SET password = md5(concat('admin', salt)) WHERE acronym = 'admin';
 UPDATE user SET password = md5(concat('pelle', salt)) WHERE acronym = 'pelle';
...
SELECT acronym, name FROM User WHERE acronym = 'pelle' AND password = md5(concat('pelle', salt));
select * from user;
UPDATE User SET password = md5(concat('doe', salt)) WHERE acronym = 'doe';
SELECT * FROM VMovie;
